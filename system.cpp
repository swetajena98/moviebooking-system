...
#include <fstream>
using namespace std;
int obn,prnt_once=0,ar[20][50][50][12][18],hall_no,hall_no_cus,hall_ent;
int dir_customer=0;
//=0 if the user directly goes to user instead of manager
//obn is the storing the selected number of the movie hall
string ticket_save,*h;
class key
{
public:
virtual void hallslt()=0;
virtual void hallpnt()=0;
};
...
int  pas_rak=0;//to store the return value of function passchk()
...
void movie_booking::hallpnt()
{
system("cls");
string *p;
p=h;
if(dir_customer!=0&&hall_no!=0)
{
   cout<<"\n\n\t\t\t\t\t\t\tTHE MOVIE HALLS IN BHUBANESWAR ARE:\n";
for(int i=0;i<hall_no;i++)
    {
           cout<<"\t\t\t\t\t\t\t";
        cout<<(*p)<<endl;
...
else if(hall_no!=0||dir_customer==0)
    {
...
{   char mov='c',in='c';
    ...
    in='c';
               ...
               cout<<"TO CHOOSE THE HALL AGAIN PRESS 9\n";
               cin>>hall_ent;
...
void movie_booking::passpro()//to protect the password
{
